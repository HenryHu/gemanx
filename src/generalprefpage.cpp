...
{	
	...
	m_IPNotifier = gtk_check_button_new_with_mnemonic (_("Display IP notifier"));
	gtk_widget_show (m_IPNotifier);
	gtk_box_pack_start (GTK_BOX (m_Widget), m_IPNotifier, FALSE, FALSE, 0);
	...
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(m_IPNotifier), AppConfig.IPNotifier);
...
void CGeneralPrefPage::OnOK()
{
	...
	AppConfig.IPNotifier = gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON(m_IPNotifier));
