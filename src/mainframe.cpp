...
CMainFrame::CMainFrame()
{
	...
	gtk_window_set_wmclass(GTK_WINDOW(m_Widget), "GeManX", "gemanx");
...
//	g_signal_connect(G_OBJECT(m_Widget), "focus-out-event", G_CALLBACK(CMainFrame::OnDeactivated), this);
	...
	GtkWidget* vsep = NULL;
#ifdef USE_IMAGEVIEW
        m_DownloadingProgressBar = (GtkProgressBar *)gtk_progress_bar_new();
	gtk_box_pack_start (GTK_BOX (m_Statusbar), (GtkWidget*)m_DownloadingProgressBar, FALSE, FALSE, 2);
...
#ifdef USE_IMAGEVIEW
                gtk_widget_hide_all( (GtkWidget *)m_DownloadingProgressBar );
#endif
	}
...
CTelnetCon* CMainFrame::NewCon(string title, string url, CSite* site )
{
	...
	m_pView->SetFont(AppConfig.FontFamily, AppConfig.FontSize, AppConfig.AntiAliasFont);
	m_pView->SetFontEn(AppConfig.FontFamilyEn, AppConfig.FontSizeEn, AppConfig.AntiAliasFont);
        m_pView->m_TermWidthBaseLine = AppConfig.TermWidthBaseLine;
        m_pView->m_TermHeightBaseLine = AppConfig.TermHeightBaseLine;
        m_pView->m_TermAspectRatio = AppConfig.TermAspectRatio;
        if (m_pView->m_TermWidthBaseLine / m_pView->m_TermAspectRatio
                > m_pView->m_TermHeightBaseLine) {
            m_pView->m_TermWidthBaseLine = 
                (int)floor(m_pView->m_TermHeightBaseLine * 
                        m_pView->m_TermAspectRatio);
        } else {
            m_pView->m_TermHeightBaseLine = 
                (int)floor(m_pView->m_TermWidthBaseLine /
                        m_pView->m_TermAspectRatio);
        }
...
GtkActionEntry CMainFrame::m_ActionEntries[] =
  {
    ...
    {"paste_with_color", GTK_STOCK_SELECT_COLOR, _("Paste with _ANSI Color"), NULL, NULL, G_CALLBACK (CMainFrame::OnPasteWithColor)},
...
static const char *ui_info =
  ...
  "      <menuitem action='paste_with_color'/>"
...
void CMainFrame::MakeUI()
{
  ...
  CreateFavoritesMenu();
	...
	gtk_status_icon_set_tooltip(m_TrayIcon, "GeMan X");
...
#include "gemanx_xpm.xpm"
#include "gemanx_inverse_xpm.xpm"
#include "conn_xpm.xpm"

void CMainFrame::LoadIcons()
{
	m_MainIcon = gdk_pixbuf_new_from_xpm_data((const char**)gemanx_xpm);
	m_InverseMainIcon = gdk_pixbuf_new_from_xpm_data((const char**)gemanx_inverse_xpm);
...
void CMainFrame::OnFont(GtkMenuItem* mitem UNUSED, CMainFrame* _this)
{
	...
	if( gtk_dialog_run((GtkDialog*)dlg) == GTK_RESPONSE_OK )
	{
		gchar* name = gtk_font_selection_dialog_get_font_name( fsdlg );
                INFO("Select Font: %s", name);

		PangoFontDescription* desc = pango_font_description_from_string( name );
		g_free( name );
		const char* family = pango_font_description_get_family(desc);
		AppConfig.FontFamily = family;
		AppConfig.FontSize = pango_font_description_get_size(desc) / PANGO_SCALE;
		pango_font_description_free(desc);

                /* Save current term size */
                if ( _this->GetCurView() ) {
                    AppConfig.TermWidthBaseLine = 
                        _this->GetCurView()->m_TermWidth;
                    AppConfig.TermHeightBaseLine = 
		...
		if( gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON(apply_to_all) ) )
		{
                    ...
                    {
                        CFont * enFont = (*it)->GetFontEnNow();
                        (*it)->SetFontEn(enFont->GetName(), enFont->GetFontSize(),
                                enFont->GetAntiAlias());;
                        (*it)->SetFont(AppConfig.FontFamily,
                                AppConfig.FontSize,
                                AppConfig.AntiAliasFont);
                        (*it)->SetBaseLine(AppConfig.TermWidthBaseLine,
                                AppConfig.TermHeightBaseLine);
                    }
                    /// FIXME: Poor design! Different connection must be allowed to use different fonts in the future.
                }
		else if( _this->GetCurView() )
			_this->GetCurView()->SetFont(AppConfig.FontFamily,
                                AppConfig.FontSize,
                                AppConfig.AntiAliasFont);
...
void CMainFrame::OnFontEn(GtkMenuItem* mitem UNUSED, CMainFrame* _this)
{
	...
	char pango_font_name[32];
        int nowFontSize = (_this->GetCurView() != NULL) ?
            (int)(_this->GetCurView()->GetFontEnNow()->GetFontSize()) :
            AppConfig.FontSizeEn;
	sprintf( pango_font_name, "%s %d", AppConfig.FontFamilyEn.c_str(), (nowFontSize > 2 && nowFontSize <= 72) ?  nowFontSize : 12 );
	gtk_font_selection_dialog_set_font_name(fsdlg, pango_font_name);
	...
	if( gtk_dialog_run((GtkDialog*)dlg) == GTK_RESPONSE_OK )
	{
		...
		AppConfig.FontSizeEn = pango_font_description_get_size(desc)/PANGO_SCALE;
		pango_font_description_free(desc);
		...
		if( gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON(apply_to_all) ) )
		{
                    ...
                    for( it = _this->m_Views.begin(); it != _this->m_Views.end(); ++it )
                    {
                        CFont * cnFont = (*it)->GetFontNow();
                        (*it)->SetFont(cnFont->GetName(), cnFont->GetFontSize(),
                                cnFont->GetAntiAlias());
                        (*it)->SetFontEn(AppConfig.FontFamilyEn,
                                AppConfig.FontSizeEn,
                                AppConfig.AntiAliasFont);
                        (*it)->SetBaseLine(AppConfig.TermWidthBaseLine,
                                AppConfig.TermHeightBaseLine);
...
void CMainFrame::OnFullscreenMode(GtkMenuItem* mitem UNUSED, CMainFrame* _this)
{
	if (_this->m_Mode != FULLSCREEN_MODE) {
		...
		if (AppConfig.ShowStatusBar)
			gtk_widget_show((GtkWidget *)_this->m_Statusbar);
...
void CMainFrame::OnSave(GtkWidget *widget,gpointer *data)
{
	...
	dialog = gtk_file_chooser_dialog_new ("Save Article",
				      NULL,
	...
	gtk_file_chooser_set_current_folder (GTK_FILE_CHOOSER (dialog), home_path);
	char newfile[100];
	sprintf (newfile,"%s.txt", _("Untitled document"));
	...
	//destroy the download dialog;
	gtk_widget_destroy (gtk_widget_get_ancestor(widget,GTK_TYPE_DIALOG));


}

void CMainFrame::OnCancel(GtkWidget *widget,gpointer *data)
{
	gtk_widget_destroy (gtk_widget_get_ancestor(widget,GTK_TYPE_DIALOG));
}
...
void CMainFrame::OnAbout(GtkMenuItem* mitem UNUSED, CMainFrame* _this)
{
	char* pcmanx_authors = _(
			...
			"Jason Xia (Developer) <jasonxh@gmail.com>\n"
	                "Haifeng Chen (Chinese Simplified (zh_CN) Translation) <optical.dlz@gmail.com>" );

        char* authors = _(
                "Ruizhe Li (Developer) <liruizhe@gmail.com>\n"
                "Henry Hu (Developer) <henry.hu.sh@gmail.com>"
                );
	...
	GtkWidget* dlg = gtk_message_dialog_new_with_markup( (GtkWindow*)_this->m_Widget,
						...
						"Bug Report: %s\n\n"
						"<b>Authors</b>:\n%s\n\n"
                                                "<b>Contributors to PCMan X</b>:\n%s\n\n"
                                                ), PACKAGE_VERSION, PACKAGE_BUGREPORT, authors, pcmanx_authors);
...
void CMainFrame::updateBBSList(GtkMenuItem* pMenuItem, CMainFrame* pThis)
{
	...
	if (g_bUpdateingBBSList == false) {
		...
		child_pid = fork();
		if (child_pid == 0) {
			int t_nRet = system("wget -O ~/.gemanx/sitelist.tmp "
				"http://free.ym.edu.tw/pcman/site_list.utf8 && "
				"mv -f ~/.gemanx/sitelist.tmp ~/.gemanx/sitelist");
			if (t_nRet == 0)
...
void CMainFrame::pasteFromClipboard(GtkMenuItem* pMenuItem UNUSED, CMainFrame* pMainFrame)
{
	...
	if (t_pView != NULL)
		t_pView->PasteFromClipboard(true, false);
}
...
{
	if(_this->GetCurView())
		_this->GetCurView()->PasteFromClipboard(false, false);
}

void CMainFrame::OnPasteWithColor(GtkMenuItem* mitem UNUSED, 
        CMainFrame* _this)
{
	if(_this->GetCurView())
		_this->GetCurView()->PasteFromClipboard(false, true);
}
...
void CMainFrame::OnPreference(GtkMenuItem* mitem UNUSED, CMainFrame* _this)
{
	...
	if (AppConfig.ShowStatusBar)
		gtk_widget_show(_this->m_Statusbar);
	else
...
#ifdef USE_LIBNOTIFY
void CMainFrame::OnNotify(NotifyNotification *notification,
                           gchar *action,
                           gpointer user_data)
{
	DEBUG("OnNotify");
	CMainFrame *_this = (CMainFrame *)user_data;
	_this->Show();
	_this->Present();
	_this->SetFocus();
}
