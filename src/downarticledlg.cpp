...
#define DOWN_ARTICLE_POLL_DELAY 1000  // in microsecond
...
{
	gsize len;
	char *buf = my_convert(line, -1, "UTF-8", enc, &len);
...
void CDownArticleDlg::DownArticleFunc(CDownArticleDlg *_this)
{
    ...
    int firstLine = con->m_FirstLine;
    int rowPerPage = con->m_RowsPerPage;
    const char * encoding = con->m_Site.m_Encoding.c_str();
    
    string * page[2];
    page[0] = new string[rowPerPage];
    page[1] = new string[rowPerPage];

    int i, j, k;
    int idThisPage = 0;
    int idLastPage = 1;

    for ( ; !isFinished; iPage++) {
        char ** screen = con->m_Screen;
        int lastLine = rowPerPage - 2;
        /* Convert the encoding of this page 
         * and detect the last line */
        for (j = firstLine; j < firstLine + rowPerPage; j++) {
            /* Covert the encoding of this line */
            gsize len;
            gchar * buf = my_convert(screen[j], -1, "UTF-8", encoding, 
			&len);
            if (buf) {
		/* Trim trailing spaces */
                while (len > 0 && (buf[len - 1] == ' ' || buf[len] == '\t'))
                    buf[len--] = '\0';
                page[idThisPage][j - firstLine] = buf;
                g_free(buf);
            } else {
                page[idThisPage][j - firstLine] = "";
            }
            /* Detect if this is last line */
            string & thisLine = page[idThisPage][j - firstLine];
            if (!isFinished &&
                    (thisLine.find("※ 来源") == 0 ||
                     thisLine.find("※ 发信站") == 0 ||
                     thisLine.find("※ 發信站") == 0)) {
                INFO("%s", thisLine.c_str());
                isFinished = true;
                lastLine = j - firstLine;
            }
        }

        string & bottom = page[idThisPage][rowPerPage - 1];
        fprintf(stderr, "A: %d\n", isFinished);
        if (bottom.find("下面还有喔") != 0 &&
                (bottom.find("瀏覽") == string::npos 
                 || bottom.find("100%") != string::npos)) {
            isFinished = true;
        }
        fprintf(stderr, "B: %d\n", isFinished);
        ...
        /* If this is the last page, check if the are duplicated lines */
        if (isFinished && iPage != 0) {
            /* i is page id of last page */
            i = rowPerPage - 2;
            /* j is page id of this page */
            j = lastLine + 1;
            /* First, find the lowest line in this page which equals
             * to the last line of last page */
            INFO("Last page: %s", page[idLastPage][i].c_str());
            while (j > 0) {
                while (j > 0) {
                    j--;
                    if (page[idThisPage][j] == page[idLastPage][i])
                        break;
                }
                /* Second, check if they are really duplicated lines */
                bool isDup = true;
                for (k = j - 1; k >= 0; k--) {
                    i--;
                    if (page[idThisPage][k] != page[idLastPage][i]) {
                        /* not really duplicated lines */
                        j = k;
                        isDup = false;
                        break;
                    }
            ...
            /* Except first page, the first line of this page
             * is duplicated */
            j = (iPage == 0) ? -1 : 0;
        }

        for (i = j + 1; i < rowPerPage - 1; i++) {
            str.append(page[idThisPage][i]);
            str.push_back('\n');
        }
        ...
        idLastPage = 1 - idLastPage; 
        idThisPage = 1 - idThisPage;

        unsigned line = con->m_LineCounter;
        con->SendRawString(" ", 1); // Space
