...
public:
    ...
    string GetCorrentContent(){return m_Content;}
	void MoveNext(int &row, int &col);
	void ClearLastPreedit();
	...
	void DownLoadArticle(GtkProgressBar *progress);
...
protected:
    ...
    void OnButtonUp(GdkEventButton *evt);
    ...
    void OnDestroy();
    static void ThreadProcessHyperlinkClicked(gpointer *arg);
...
#endif
	int m_LastPreeditRow, m_LastPreeditCol, m_LastPreeditLen;
};
