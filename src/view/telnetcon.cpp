	...
	: CTermData(pView), m_Site(SiteInfo)
{
	...
	{
		if( (locale_str = my_convert(m_Site.GetPreLoginPrompt().c_str(), 
	...
	{
		if( (locale_str = my_convert(m_Site.GetLoginPrompt().c_str(), 
	...
	{
		if( (locale_str = my_convert(m_Site.GetPasswdPrompt().c_str(), 
...
bool CTelnetCon::Connect()
{
	...
	PreConnect( address, m_Port, useSSH );
	...
	/* external telnet */
	if ( !useSSH && m_Site.m_UseExternalTelnet )
	{
		...
		if ( m_Pid == 0 )
		{
			...
			execlp ( prog, prog, "-8", address.c_str(), strPort, NULL ) ;
			exit(EXIT_FAILURE);
		}
		else
		{
	...
	/* external ssh */
	else if ( useSSH && m_Site.m_UseExternalSSH )
	{
		// Suggestion from kyl <kylinx@gmail.com>
		...
		if ( m_Pid == 0 )
		{
			...
			close(m_SockFD);
                        ...
                        sprintf(strPort, "%d", m_Port);
            if (useSSH == 2)
              execlp ( prog, prog, address.c_str(), "-p", strPort, NULL ) ;
            else
              execlp ( prog, prog, "-1", address.c_str(), "-p", strPort, NULL ) ;
			exit(EXIT_FAILURE);
		}
		else
		{
...
static gboolean OnReconnectTimer(gpointer data)
{
	CTelnetCon* tcon = (CTelnetCon*)data;
	tcon->m_Site.m_ReconnectTimerId = 0;
	tcon->Reconnect();
	return FALSE;
}
...
void CTelnetCon::OnClose()
{
	...
	{
		m_Site.m_ReconnectTimerId = g_timeout_add_seconds(m_Site.m_ReconnectInt, OnReconnectTimer, (gpointer)this);
	}
}
...
{
    INFO("on bell timer");
    if (_this->m_IsFirstLineModified) {
        INFO("Msg is on first line");
        int iLine = _this->m_FirstLine;
        int lastLine = _this->m_FirstLine + _this->m_RowsPerPage;
        short bgcolor = _this->GetLineAttr(iLine)[0].GetBackground();
        string msgtext(_this->m_Screen[iLine], strchr(_this->m_Screen[iLine], ' '));
        msgtext.append(" ");
        for ( iLine = iLine + 1; iLine < lastLine && 
                _this->GetLineAttr(iLine)[0].GetBackground() == bgcolor;
                iLine++) {
            msgtext.append(_this->m_Screen[iLine]);
        }
        gsize l;
        gchar *utf8_text = my_convert(
                msgtext.c_str(), msgtext.length(), 
                "UTF-8", _this->m_Site.m_Encoding.c_str(), &l);
        ...
        _this->m_IsFirstLineModified = false;
    }
    else if( _this->m_IsLastLineModified ) {
        INFO("Msg is on last line");
        char* line = _this->m_Screen[ _this->m_FirstLine+_this->m_RowsPerPage-1 ];
...
void CTelnetCon::PreConnect(string& address, unsigned short& port, int& useSSH)
{
	m_Duration = 0;
	m_IdleTime = 0;
	m_State = TS_CONNECTING;
        ...
        int p = m_Site.m_URL.find("://", 0);
        if (p >=0 ) //use protocol identifier
        {
            string protocol = m_Site.m_URL.substr(0, p);
            if (protocol.compare("ssh") == 0) {
                useSSH = 2;
                port = 22;
            } else if (protocol.compare("ssh1") == 0) {
                useSSH = 1;
                port = 22;
            }
            newURL = m_Site.m_URL.substr(p+3, m_Site.m_URL.length() - p - 3);
        } else {
            newURL = m_Site.m_URL;
        }

	p = newURL.find(':',true);
	if( p >=0 )		// use port other then 22 & 23;
	{
		port = (unsigned short)atoi(newURL.c_str()+p+1);
		address = newURL.substr(0, p);
	}
	else
		address = newURL;
}
...
void CTelnetCon::Close()
{
	...
	if (m_Site.m_ReconnectTimerId != 0)
	{
		g_source_remove(m_Site.m_ReconnectTimerId);
		m_Site.m_ReconnectTimerId = 0;
	}
}
...
void CTelnetCon::OnLineModified(int row)
{
	...
	if( row == (m_FirstLine+m_RowsPerPage-1) )	// If last line is modified
            m_IsLastLineModified = true;
        if (row == m_FirstLine)
            m_IsFirstLineModified = true;
}
...
#ifdef USE_LIBNOTIFY
static gboolean notify_supports_actions()
{
	GList *caps;
	gint i = 0;
	...
	do {
		if (g_strcmp0((char*)caps->data, "actions") == 0)
		{
			g_list_free(caps);
			return TRUE;
		}
...
static void on_notify_closed(NotifyNotification *n, gpointer user_data)
{
...
void CTelnetCon::OnNewIncomingMessage(const char* line)	// line is already a UTF-8 string.
{
	...
	if( bot && use_nancy )
	{
		...
		size_t n = str.find_first_of("  ");  // cut userid and spaces at head
	...
	NotifyNotification *notification =
	  notify_notification_new(
				  ...
				  NULL);
#endif
    // can we use || to prevent duplication?
#else
	NotifyNotification *notification =
	  notify_notification_new(
				  summary,
	...
	notify_notification_set_icon_from_pixbuf(notification,
						 mainfrm->GetMainIcon());
	// Some implementations such as Ubuntu's notify-osd
	// do not support actions
	if (notify_supports_actions())
	{
		notify_notification_add_action(notification,
			"default", "default",
			(NotifyActionCallback)mainfrm->OnNotify, mainfrm,
			NULL);
	}
	g_signal_connect(G_OBJECT(notification), "closed",
			G_CALLBACK(on_notify_closed), NULL);
