...
{
    CTermCharAttr pAttr;
    memcpy(&pAttr, &val2, sizeof(short));
    pAttr.m_CharSet = m_CharSet;
    pAttr.m_NeedUpdate = m_NeedUpdate;
    return pAttr.AsShort() == this->AsShort();
}
...
/*inline bool isimage(const char *p)
{
	if(!strncasecmp(p,"jpg",3)||!strncasecmp(p,"png",3)||!strncasecmp(p,"bmp",3)||!strncmp(p,"JPG",3)||!strncmp(p,"PNG",3))
		return true;
	else
		return false;
}*/

/* Modified by Li Ruizhe 2009.4.16 */
inline void DetectEMails( CTermData * data, int firstLine, int rowsPerPage,
       int colsPerPage )
{
    int ilink = 0, stage = 0;
    int iline = firstLine;
    int ilast_line = iline + rowsPerPage;
    char ** screen = data->m_Screen;
    for (int i = firstLine * colsPerPage; i < ilast_line * colsPerPage;
            i += (CTermCharAttr::CS_ASCII == 
                data->GetLineAttr(i/colsPerPage)[i%colsPerPage].GetCharSet()?1:2)) {
            unsigned char ch = screen[i/colsPerPage][i%colsPerPage];
            switch( stage )
            {
                ...
                case 3:	//  This is a valid URL.
                    if( !isurl(ch) )
                    {
                        for( ; ilink < i; ilink++ )
                        {
                            data->GetLineAttr(ilink/colsPerPage)[ilink%colsPerPage]
                                .SetHyperLink(true);
                            data->GetLineAttr(ilink/colsPerPage)[ilink%colsPerPage]
                                .SetNeedUpdate(true);
                        }
...
/* Modified by Li Ruizhe 2009.4.16 */
inline void DetectCommonURLs( CTermData * data, int firstLine, int rowsPerPage,
       int colsPerPage )
{
    int ilink = 0, stage = 0;
    int iline = firstLine;
    ...
    for (int i = firstLine * colsPerPage; i < ilast_line * colsPerPage;
            ...
            switch( stage ) {
                case 0:	// a url scheme character is found, beginning of url.
                    ...
                    break;
                case 1:	// "://" is found.
                    if ((':' == screen[i/colsPerPage][i%colsPerPage]) &&
                            ('/' == screen[(i+1)/colsPerPage][(i+1)%colsPerPage]) &&
                            ('/' == screen[(i+2)/colsPerPage][(i+2)%colsPerPage]) &&
                            isurl(screen[(i+3)/colsPerPage][(i+3)%colsPerPage])) {
                        stage = 2;
                        i += 3;
                    }
...
void CTermData::DetectHyperLinks()
{
	...
	for( ; iline < ilast_line; iline++ )
	{
        ...
        DetectEMails(this, m_FirstLine, m_RowsPerPage, m_ColsPerPage);
        DetectCommonURLs(this, m_FirstLine, m_RowsPerPage, m_ColsPerPage);
}
...
static void read_line_and_color( int row, int col1, int col2, void* data )
{
	ReadStatus* rs = (ReadStatus*)data;
	string* text = rs->text;
        vector<CTermCharAttr> * vAttr = rs->vAttr;

	if ( rs->lines )
	{
		*text += rs->eol;
                vAttr->push_back(CTermCharAttr());
	...
	// GetLineAndColor
	{
		...
		for ( i = col1; i < col2; i++ )
		{
                    line.push_back(pLine[i]);
		}

		// if current background is black,
                size_t n;
		while ( (n = line.length()) && line[n-1] == ' ' &&
                        pAttr[n-1].IsSameAttr(attr.AsShort())) {
...
string CTermData::GetText( CTermSelection* sel, bool trim )
{
	string text;
	int endrow;
	ReadStatus rs = { this, &text, NULL, 0, "\n" };

        fprintf(stderr, "%d\n", trim);
        fprintf(stderr, "[1] (%d, %d)\n", sel->m_End.row, sel->m_End.col);
...
string CTermData::GetTextWithAttr(vector<CTermCharAttr> * vAttr, CTermSelection* sel, bool trim)
{
    string text;
    int endrow;
    ReadStatus rs = { this, &text, vAttr, 0, "\n" };
    ...
    sel->ForEachLine( read_line_and_color, (void*)&rs );
...
string CTermData::GetSelectedTextWithAttr(vector<CTermCharAttr> * vAttr, bool trim)
{
    return GetTextWithAttr(vAttr, m_Sel, trim);
}

string CTermData::GetSelectedText(bool trim)
{
	return GetText( m_Sel, trim );
}
