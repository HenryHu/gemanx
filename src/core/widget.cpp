...
void CWidget::SetTooltip(char *tooltip)
{
#if GTK_CHECK_VERSION(2,12,0)
	if (!tooltip && !m_Tooltip)
		return;
	if (tooltip && m_Tooltip && (strncmp(tooltip, m_Tooltip, 255) == 0))
		return;
	gtk_widget_set_tooltip_text(m_Widget, NULL);
	if (tooltip)
	{
		gtk_widget_set_tooltip_text(m_Widget, tooltip);
