...
public:
    ...
    virtual void OnPreedit();
    int DrawChar(int row, int col);
	void DrawCharAt(gunichar ch, int row, int col, int cwid);
	...
	virtual void OnButtonDown(GdkEventButton* evt);
	virtual void OnButtonUp(GdkEventButton *evt);
    void PasteFromClipboard(bool primary, bool with_color);
    ...
    void SetFont( string name, int pt_size, bool anti_alias);
    void SetFontFamily(string name);
    void SetFont(CFont* font);
    void SetFontEn( string name, int pt_size, bool anti_alias);
    ...
    CFont* GetFontNow(){	return m_FontNow;	}
    CFont* GetFontEnNow(){	return m_FontEnNow;	}
    ...
    void SetBaseLine(int width, int height) { 
        m_TermWidthBaseLine = width;
        m_TermHeightBaseLine = height;
        RecalcCharSize();
    }
...
protected:
    ...
    void RecalcCharSize();
    void UpdateCaretPos();
    bool HyperLinkHitTest(int x, int y, char * buf);
    void OnDestroy();
...
protected:
	...
	bool m_bVerticalCenterAlign;
        ...
        int m_TermWidth;
        int m_TermHeight;
        int m_TermWidthBaseLine;
        int m_TermHeightBaseLine;
        double m_TermAspectRatio;
    ...
    static vector<CTermCharAttr> m_s_ANSIColorAttr;
