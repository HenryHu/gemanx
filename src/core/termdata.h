...
public:
	...
	//string GetSelectedTextWithColor( bool trim = true );
        string GetSelectedTextWithAttr(vector<CTermCharAttr> * vAttr, bool trim = true);

	string GetText( CTermSelection* sel, bool trim );
        string GetTextWithAttr(vector<CTermCharAttr> * vAttr, CTermSelection* sel, bool trim);
...
//		printf ("getting line:%s\n",pLine);
        ...
        /* Add by Li Ruizhe 2009.4.16 */
        inline CTermCharAttr* GetLineAttr(int row)
        {
            return (CTermCharAttr* )(m_Screen[row] + m_ColsPerPage + 1);
        }
        /* End by Li Ruizhe 2009.4.16 */
	...
	inline string GetAllText( bool trim = true )
	{
		...
		sel.SelectAll();
		return GetText( &sel, trim );
	}
	//The same as GetAllText but get color too.
	inline string GetAllTextWithColor( vector<CTermCharAttr> * vAttr, bool trim = true )
	{
		CTermSelection sel( this );
		sel.SelectAll();
		return GetTextWithAttr ( vAttr, &sel, trim );
	}
	...
	inline char* AllocNewLine( const int ColsPerPage )
	{
		...
		char * NewLine = new char[ 1 + ColsPerPage * ( 1 + sizeof( CTermCharAttr ) ) ];
