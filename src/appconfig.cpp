...
CAppConfig::CAppConfig() : CConfigFile("gemanx") /*, HyperLinkColor(255,102,0)*/
{
}
...
//		CFG_STR ( Shadow )
	...
	BEGIN_CONFIG_SECT(Display)
                CFG_INT ( TermWidthBaseLine )
                CFG_INT ( TermHeightBaseLine )
                CFG_DOUBLE ( TermAspectRatio )
		CFG_STR ( FontFamily )
		CFG_INT ( FontSize )
	...
	BEGIN_CONFIG_SECT(Site)
		...
		_CFG_INT ( "ReconnectInt", m_DefaultSite.m_ReconnectInt )
...
//		BLOWFISH_CTX* bfc = NULL;
		while( fgets( line, 1024, fi ) )
		{
			...
			else if( 0 == strcmp( pname, "ReconnectInt" ) )
				pSite->m_ReconnectInt = atoi(pval);
...
//	m_IsLoggedIn = false;
	...
	BeepOnBell = true;
        ...
        TermWidthBaseLine = 1024;
        TermHeightBaseLine = 616;
        TermAspectRatio = 1.66;
	FontSize = 15;
	FontFamily = "AR PL UMing TW MBE";
	FontSizeEn = 14;
	FontFamilyEn = "Bitstream Vera Sans Mono";
	AntiAliasFont = true;
	CompactLayout = false;
	HorizontalCenterAlign = false;
	VerticalCenterAlign = false;

	MailClient = WebBrowser = "www-browser %s";
#ifdef USE_IMAGEVIEW
	ImageViewer = "eog %s";
#endif
