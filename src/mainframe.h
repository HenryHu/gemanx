...
public:
	...
	static void OnPasteWithColor(GtkMenuItem* mitem, CMainFrame* _this);
...
#ifdef USE_LIBNOTIFY
	static void OnNotify(NotifyNotification *notification, gchar *action, gpointer user_data);
#endif
...
#ifdef USE_IMAGEVIEW
        void ShowDownloadProcessBar() { gtk_widget_show_all( GTK_WIDGET( m_DownloadingProgressBar )); };
        void HideDownloadProcessBar() { gtk_widget_hide_all( GTK_WIDGET( m_DownloadingProgressBar )); };
#endif
...
#ifdef USE_IMAGEVIEW
        GtkProgressBar * m_DownloadingProgressBar;
#endif
